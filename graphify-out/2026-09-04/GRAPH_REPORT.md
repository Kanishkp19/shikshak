# Graph Report - shikshak-ai  (2026-09-04)

## Corpus Check
- 588 files · ~482,604 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5872 nodes · 8633 edges · 418 communities (340 shown, 69 thin omitted)
...
1. `get_client()` - 53 edges
2. `require_playable_video()` - 49 edges
3. `CamelModel` - 47 edges
4. `render_svg_frames_to_mp4()` - 42 edges
5. `escape_xml()` - 31 edges
6. `cn()` - 31 edges
7. `call_llm_with_retry()` - 28 edges
8. `_make_project()` - 27 edges
9. `run_session_pipeline()` - 27 edges
10. `VisualBrief` - 27 edges

## Surprising Connections (you probably didn't know these)
- `test_avatar_presenter_modes()` --uses--> `AvatarPresenterMode`  [INFERRED]
  apps/api/tests/test_musetalk_avatar.py → apps/api/models/avatar.py
...
Cohesion: 0.03
Nodes (107): create_learning_path(), delete_session(), _format_video_url(), generate_next_segment(), get_document(), get_learner_profile(), get_learning_path(), get_related_concepts() (+99 more)
...
Cohesion: 0.10
Nodes (26): ABC, Exception, Shikshak AI — video generation provider base. Every video provider (FlowCache,…, Every video provider implements this interface., Render or retrieve a video for the request and return a VideoGenerationResult., Return False if the provider can't be used right now., Request contract for video generation providers., Result returned by video generation providers. (+18 more)

### Community 7 - "supabase_persistence.py"
Cohesion: 0.07
Nodes (55): generate_deep_dive(), Any, task, Shikshak AI — Deep Dive Generation Agent. Single responsibility: when the user…, Generate a deep-dive segment for the given concept. Returns a segment dict with…, run(), create_session(), Create a new teaching session. Triggers the Orchestrator → agent DAG. (+47 more)
...
Cohesion: 0.12
Nodes (20): dispatch_scene_render(), get_renderer_for_mode(), _infer_legacy_visual_type(), Any, Path, task, Shikshak AI — Visual Selection & Dispatch Router. Single responsibility: 1.…, Walk every segment and scene, verifying that visual modes are valid and ready… (+12 more)

### Community 12 - "render_motion_canvas_spec"
Cohesion: 0.06
...
Cohesion: 0.09
Nodes (27): LessonPlan, plan_lesson(), Any, BaseModel, task, Shikshak AI — Lesson Planning Agent (v3). Two modes: document mode: uses…, Return a validated lesson plan as a dict. When pdf_structure is provided…, run() (+19 more)
...
### Community 28 - "test_content_scriptwriter.py"
Cohesion: 0.10
Nodes (26): explain_segment(), explain_segment_narration_only(), explain_segment_with_blueprint(), _format_chunks(), Any, task, Shikshak AI — Explanation Agent (v2). Uses the unified ContentBlueprint from…, Celery task — returns narration script only for backward compat. (+18 more)
...
### Community 31 - "media.py"
Cohesion: 0.12
Nodes (26): AvatarPresenterMode, Pedagogical teacher visibility and layout modes., MediaInfo, MediaValidationError, probe_media(), Path, Small, strict media probes used at each video-pipeline boundary. The pipeline…, Raised when an expected media asset is absent or not decodable. (+18 more)
...
Cohesion: 0.06
Nodes (40): get_supabase_client(), main(), populate_video_cache(), Path, build_cache_prompt(), cached_or_compute(), hash_prompt(), make_prompt_hash() (+32 more)
...
Cohesion: 0.12
Nodes (27): compute_cache_key(), ImageCache, Shikshak AI — Image Cache for Educational Asset Generation. Caches synthesized…, Derive a deterministic SHA-256 hash for the image generation parameters., Local filesystem cache for synthesized visual assets., create_fallback_chalkboard_canvas(), generate_educational_image(), ImageStyle (+19 more)
...
Cohesion: 0.18
Nodes (20): _build_diagram_from_scenes(), ContentBlueprint, DiagramSpecBlueprint, _extract_narration_from_scenes(), _fallback_blueprint(), generate_content_blueprint(), BaseModel, Shikshak AI — Content Scriptwriter Agent (v3). Generates a **single unified… (+12 more)
...
### Community 124 - "AvatarProfile"
Cohesion: 0.05
Nodes (41): task, Shikshak AI — Avatar Rendering Agent. Single responsibility: take the…, Return the path to the rendered avatar video file., render(), run(), AvatarProfile, BaseModel, Path (+33 more)

### Community 125 - "knowledge_retrieval.py"
Cohesion: 0.05
Nodes (56): Any, BaseModel, task, QueryRewrite, Shikshak AI — Knowledge Retrieval Agent. Single responsibility: given a…, Return top_k chunks for the query, filtered by CRAG relevance gate., Subclass of list that carries CRAG gating metadata while behaving as a plain…, Compute combined similarity + lexical overlap score for each chunk. (+48 more)
...
### Community 127 - "visual_director.py"
Cohesion: 0.22
...
### Community 136 - "validate_visual_payload"
Cohesion: 0.16
...
Cohesion: 0.10
Nodes (26): _fallback_scenes(), plan_scenes_for_segment(), Any, BaseModel, task, Shikshak AI — Scene Planning Agent (Universal Multi-Subject). Single…, Generate 3-5 typed scenes for a single lesson segment across any subject., Validate each scene against its visual_mode payload schema and normalize. (+18 more)

### Community 140 - "RuntimeError"
Cohesion: 0.18
Nodes (14): main(), CLI entry point for skill-comply., generate_scenarios(), Path, Generate pressure scenarios from skill + spec using LLM., Generate 3 scenarios with decreasing prompt strictness. Calls claude -p with…, generate_spec(), Path (+6 more)
...
### Community 142 - "require_playable_video"
Cohesion: 0.11
Nodes (26): Any, Path, Render an AI illustration scene with smooth kinetic camera push-in and clean…, render_ai_illustration(), Any, Path, Render animated typographic concept card with progressive key point…, render_kinetic_text() (+18 more)

### Community 143 - "quality_gate.py"
Cohesion: 0.11
Nodes (26): audit_cinematic_video(), audit_scene(), audit_scenes_for_segment(), compute_visual_necessity_score(), Any, BaseModel, Path, QualityIssue (+18 more)
...
Cohesion: 0.06
Nodes (48): Any, Path, Render an anatomical_structure scene to MP4., render_anatomical_structure(), Any, Path, Render a bio_cellular_process scene to MP4., render_bio_cellular_process() (+40 more)
...
Cohesion: 0.18
Nodes (21): classify_events(), _parse_classification(), Classify tool calls against compliance steps using LLM., Classify which tool calls match which compliance steps. Returns {step_id:…, Parse LLM classification output into {step_id: [event_indices]}., _check_temporal_order(), ComplianceResult, Grade observation traces against compliance specs using LLM classification. (+13 more)
...
Cohesion: 0.10
Nodes (21): Detector, parse_spec(), parse_trace(), Path, Parse a JSONL observation trace file into sorted events., Parse a YAML compliance spec file., Step, compliant_trace() (+13 more)
...
### Community 252 - "_promote_specific"
Cohesion: 0.17
...
### Community 264 - "video_compositing.py"
Cohesion: 0.18
Nodes (16): composite_lesson(), composite_lesson_task(), composite_scenes(), composite_scenes_task(), composite_segment(), composite_segment_task(), task, Shikshak AI — Video Compositing Agent. Single responsibility: combine avatar +… (+8 more)
...
### Community 297 - "generate_with_fallback"
Cohesion: 0.27
...
### Community 355 - "manim_provider.py"
Cohesion: 0.29
Nodes (8): _build_scene_script(), _find_rendered_video(), Path, Shikshak AI — Manim video generation provider (default). Deterministic, zero-…, Last-resort: tiny valid mp4 placeholder., Generate the source code for a Manim Scene describing this concept., Render a Manim scene for the given concept., _write_placeholder_video()
...
## Knowledge Gaps
- **2594 isolated node(s):** `fs`, `path`, `resolved`, `contextDirPath`, `projects` (+2589 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 3688 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **69 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `require_playable_video()` connect `require_playable_video` to `generate_educational_image`, `models/__init__.py`, `factory.py`, `VisualBrief`, `video_compositing.py`, `generate_with_fallback`, `visual_selection.py`, `render_motion_canvas_spec`, `test_content_scriptwriter.py`, `quality_gate.py`, `render_svg_frames_to_mp4`, `AvatarProfile`, `diagram_provider.py`, `media.py`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `run_scenario()` connect `runner.py` to `_parse_stream_json`, `RuntimeError`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `get_client()` connect `supabase_persistence.py` to `models/__init__.py`, `assessment.py`, `RuntimeError`, `content_ingestion.py`, `learning_path.py`, `chunking_embedding.py`, `learner_profile.py`?**
